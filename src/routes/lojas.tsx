    ...
    <div className="p-6 lg:p-8 space-y-6">
      ...
      <Card className="border-border/60 shadow-sm">
        <CardHeader>
          ...
          <p className="mt-1 text-xs text-muted-foreground">Faturamento - Custo, acumulado no período</p>
        </CardHeader>
        <CardContent>
          ...
          <Card key={s.nome} className="border-border/60 shadow-sm">
            <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2">
              ...
              <Badge variant="outline" className="border-success/30 bg-success/10 text-success">
                ...
                {((s.margem / s.fatura) * 100).toFixed(0)}%
              </Badge>
            </CardHeader>
            <CardContent className="space-y-3 pt-2">
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <p className="font-semibold text-foreground tabular-nums">{s.entregas.toLocaleString("pt-BR")}</p>
                </div>
                <div className="flex items-center gap-1">
                  <Users className="h-3.5 w-3.5 text-muted-foreground" />
                  <div>
                    <p className="text-xs text-muted-foreground">Entregadores</p>
                    <p className="font-semibold text-foreground tabular-nums">{s.numEntregadores}</p>
                  </div>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Faturamento</p>
                  <p className="font-semibold text-foreground tabular-nums">{fmt(s.fatura)}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Custo</p>
                  <p className="font-semibold text-foreground tabular-nums">{fmt(s.custo)}</p>
                </div>
              </div>
              <div className="border-t border-border/60 pt-2">
                <p className="text-xs text-muted-foreground">Margem</p>
                <p className="font-semibold text-success tabular-nums">{fmt(s.margem)}</p>
              </div>
            </CardContent>
          </Card>
        ))}
